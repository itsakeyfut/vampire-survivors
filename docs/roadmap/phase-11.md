...
### 目的
ゲームの全画面のUIを設計書通りに実装する。HUD・タイトル・ゲームオーバー・勝利画面・レベルアップカードをBevyのUIシステムで構築する。すべてのUI実装は **vs-ui クレート**（`app/ui/src/`）に配置する。
...
**実装ファイル**: `app/ui/src/title.rs`（**vs-ui** クレート）
...
**実装ファイル**: `app/ui/src/pause.rs`（**vs-ui** クレート）
...
**実装ファイル**: `app/ui/src/game_over.rs`（**vs-ui** クレート）
...
**実装ファイル**: `app/ui/src/victory.rs`（**vs-ui** クレート）
